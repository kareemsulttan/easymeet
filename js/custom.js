$('.toggle-password').on('click', function() {
    ...
    $('[data-toggle="offcanvas"], .navToggle').on('click', function () {
        ...
        $('.fixed-top .modal-header .close').on('click', function() {
            $('.offcanvas-collapse').removeClass('open');
            if ( $( '.offcanvas-collapse' ).hasClass( "open" ) ) {
                $( '.navToggle' ).html("Hide details")
            } else (
                $( '.navToggle' ).html("Show details")
            )
        });
        if ( $( '.offcanvas-collapse' ).hasClass( "open" ) ) {
            $( '.navToggle' ).html("Hide details")
  ...
  function showPass1() {
    var inputPass1 = document.getElementById("resetpassInput1");
  ...
  function showPass2() {
    var inputPass2 = document.getElementById("resetpassInput2");
  ...
  var popup = document.getElementById("notifyPopup");
  var hidNotify = document.getElementById("dismiss");
  function notiFy() {
    var notifyBTN = document.getElementsByClassName("notify");
    popup.classList.toggle("show");
    notifyBTN.disabled = true;
    $(notifyBTN).attr("disabled", true);
  };

  // remove notification on click on dismiss
  $( hidNotify ).click(function() {
    $( popup ).remove();
  });

  // avilability status

  $('#avilabilityChk').click(function()
  {
      if ($(this).is(":checked")) {
          $('#avilabilityStatus1').modal('show');
      }else {
          $('#avilabilityStatus1').modal('hide');
      }
  });

  $('#modalAvilabilityChk').click(function()
  {
      if ($(this).is(":checked")) {
          $('#avilabilityStatus2').modal('show');
          $('#avilabilityStatus1').modal('hide');
      }else {
          $('#avilabilityStatus2').modal('hide');
          $('#avilabilityStatus1').modal('show');
      }
  });
  $('#modalAvilabilityChk2').click(function()
  {
...
; (function (window, $, undefined) {
  ...
  function profilePicture(cssSelector, imageFilePath, options) {
      ...
      function registerDropZoneEvents() {
           ...
           */
          self.element.on('click', '.photo--empty .photo__frame, .dragText', function (e) {
